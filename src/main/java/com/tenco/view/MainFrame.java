...
public class MainFrame extends JFrame {
    ...
    public MainFrame(Members loginUser) {
        ...
        // 중앙: 권한에 따른 탭Pane 구성 (중복 코드 제거 및 정확한 권한 분기)
        JTabbedPane tabbedPane = new JTabbedPane();

        // 1. 공통 탭 (강의 목록) - 관리자일 경우 수강신청 버튼 비활성화 처리됨
        ...
        if (!loginUser.isAdmin()) {
            ...
            tabbedPane.addTab("내 학적 정보", createStudentProfilePanel()); // 👈 이 코드를 추가해주세요!
        } else {
            // 관리자 계정일 때
    ...
    private JPanel createMyInfoPanel() {
        ...
        String[] regCols = {"회원ID", "학생이름", "강의ID", "강의명"};
        ...
        JPanel btnP = new JPanel(); btnP.add(cancelBtn);
        ...
        Runnable loadMyData = () -> {
            ...
            List<Registration> regs = registrationService.getMyLectureList(loginUser.getId());
            if (regs != null) {
                for (Registration r : regs) {
                    regModel.addRow(new Object[]{r.getMemberId(), r.getMemberName(), r.getLectureId(), r.getLectureName()});
                }
            }
        ...
        cancelBtn.addActionListener(e -> {
            ...
            int lecId = (int) regModel.getValueAt(row, 2);
            try {
        ...
        table.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                ...
                if (row != -1) {
                    ...
                    profF.setText(model.getValueAt(row, 3).toString() != null ? model.getValueAt(row, 3).toString() : "");
    ...
    private JPanel createAdminMemberPanel() {
        ...
        JTextField memberIdF = new JTextField(6); // 학번 (memberId)
        JPasswordField passwordF = new JPasswordField(6); // 비밀번호
        JTextField nameF = new JTextField(5);     // 이름
        JTextField phoneF = new JTextField(7);    // 전화번호
        JTextField majorF = new JTextField(6);    // 학과
        JTextField gradeF = new JTextField(2);    // 학년
                ...
                if (list != null) {
                    for (Members m : list) {
                        model.addRow(new Object[]{
                                ...
                                m.getPhone(), m.getMajor(), m.getGrade()
                        });
                    }
                }
            } catch (Exception e) {
        ...
        // 2. [학생 정보 수정] 버튼 이벤트 (이름, 전화번호, 학과 등 개별 수정 서비스 활용)
        updateBtn.addActionListener(e -> {
            int row = table.getSelectedRow();
            if (row == -1) {
    ...
    // 학생 본인 학적 정보 조회 및 비밀번호 변경 패널 (디자인 개선)
    private JPanel createStudentProfilePanel() {
        JPanel panel = new JPanel(new GridBagLayout()); // 중앙 정렬을 위한 GridBagLayout
        ...
        Runnable loadProfile = () -> {
            try {
                ...
                if (m != null) {
                    ...
                    phoneLabel.setText(m.getPhone() != null ? m.getPhone() : "-");
